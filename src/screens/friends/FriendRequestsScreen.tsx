...
type FriendRequestsNavigationProp = NativeStackNavigationProp<
  any,
  "FriendRequests"
>;
  ...
  const acceptRequest = useCallback(
    async (request: FriendRequest) => {
      ...
      try {
        ...
        showSuccessAlert(
          `You are now friends with ${request.user.displayName}!`,
          "Friend Added!",
        );
      } catch (error) {
    ...
    >
      ...
      {isLoading ? (
        ...
        <View className="flex-1 justify-center items-center px-8">
          <Ionicons
            name={
              activeTab === "incoming" ? "mail-outline" : "paper-plane-outline"
            }
            size={64}
